...
# indexing, x[i]
abc = list("abc".elems())
assert.fails(=> abc[-4], "list index -4 out of range \\[-3:2]")
assert.eq(abc[-3], "a")
assert.eq(abc[-2], "b")
assert.eq(abc[-1], "c")
assert.eq(abc[0], "a")
assert.eq(abc[1], "b")
assert.eq(abc[2], "c")
assert.fails(=> abc[3], "list index 3 out of range \\[-3:2]")
...
assert.fails(=> [1, 2] + (3, 4), "unknown.*list \\+ tuple")
assert.fails(=> (1, 2) + [3, 4], "unknown.*tuple \\+ list")
...
assert.eq([x for x in [1, 2] if => None], [1, 2])
assert.eq([x for x in [1, 2] if (=> 3 if True else 4)], [1, 2])
...
# list.pop
x4 = [1, 2, 3, 4, 5]
assert.fails(=> x4.pop(-6), "index -6 out of range \\[-5:4]")
assert.fails(=> x4.pop(6), "index 6 out of range \\[-5:4]")
assert.eq(x4.pop(), 5)
...
assert.fails(=> [3, 1, 4, 1].remove(42), "remove: element not found")

# list.index
bananas = list("bananas".elems())
assert.eq(bananas.index("a"), 1)  # bAnanas
assert.fails(=> bananas.index("d"), "value not in list")
...
assert.fails(=> bananas.index("n", -2), "value not in list")
assert.eq(bananas.index("s", -2), 6)  # bananaS
assert.fails(=> bananas.index("b", 1), "value not in list")

# start, end
assert.eq(bananas.index("s", -1000, 7), 6)  # bananaS
assert.fails(=> bananas.index("s", -1000, 6), "value not in list")
assert.fails(=> bananas.index("d", -1000, 1000), "value not in list")
