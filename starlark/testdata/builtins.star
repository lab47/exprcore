...
assert.eq(sorted(pairs, key=x => x[0]),
          [(1, 5),
           ...
           (3, 1), (3, 4), (3, 7),
           (4, 0), (4, 2)])
assert.fails(=> sorted(1), 'sorted: for parameter iterable: got int, want iterable' )
...
assert.fails(=> range(3000000000), "3000000000 out of range") # signed 32-bit values onl )
...
assert.fails(=> range(0) < range(0), "range < range not implemented" )
# <number> in <range>
assert.contains(range(3), 1)
assert.contains(range(3), 2.0)    # acts like 2
assert.fails(=> True in range(3), "requires integer.*not bool") # bools aren't number )
assert.fails(=> "one" in range(10), "requires integer.*not string" )
...
assert.fails(=> range(0, 0, 2)[:][0], "index 0 out of range: empty range" )
...
assert.fails(=> zip(z1, 1), "zip: argument #2 is not iterable: int")
z1.append(3)
...
assert.fails(=> getattr(hf, "x"), "no .x field or method")
...
assert.fails(=> getattr(myset, "onion"), "no .onion field or method")
...
hf.three = 3
hf.forty_five = 45
assert.fails(=> hf.One, 'no .One field.*did you mean .one')
assert.fails(=> hf.oone, 'no .oone field.*did you mean .one')
assert.fails(=> hf.FortyFive, 'no .FortyFive field.*did you mean .forty_five')
assert.fails(=> hf.trhee, 'no .trhee field.*did you mean .three')
assert.fails(=> hf.thirty, 'no .thirty field or method$') # no suggestion
