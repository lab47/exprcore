...
assert.fails(=> x + y, "unknown.*: set \\+ set")
...
assert.fails(=> set() | [], "unknown binary op: set | list")
...
assert.fails(=> x < y, "set < set not implemented")
...
assert.fails(=> x[0], "unhandled.*operation")
